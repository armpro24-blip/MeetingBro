...
from meetingbro.asr.streaming_whisper_adapter import StreamingWhisperAdapter  # noqa: E402
...
def _build_streaming_report(
    ...
    L: list[str] = []
    L.append("# MeetingBro Streaming-Whisper Preview Lane Benchmark")
    L.append("")
    L.append("> Generated by `scripts/benchmark_latency_accuracy_frontier.py --streaming-preview`.")
    L.append("> Measures the `whisper-streaming` (LocalAgreement-2) preview lane vs the formal Whisper lane")
    L.append("> on the real AMI meeting fixture (`ami_en2002a`, ~120 s single-distant-mic conversational English).")
    L.append("")
    L.append("## Configuration")
    L.append("")
    L.append(f"- fixture: `{fixture.get('id')}` ({fixture.get('language')}, {fixture.get('source', 'n/a')})")
    L.append(f"- Whisper: `{args.model_size}` / `{args.device}` / `{args.compute_type}` / beam `{args.beam_size}`")
    L.append(f"- knob point: pre_vad_max_seconds={args.pre_vad_max}  asr_accumulation_seconds={args.accumulation}")
    L.append("- streaming lane: `StreamingWhisperAdapter` (LocalAgreement-2, shared formal model, no second load)")
    L.append("- latency definition: audio-end → segment/preview event emitted (realtime replay)")
    L.append("")
    L.append("## Results")
    L.append("")
    L.append("| lane | p50 (s) | p95 (s) | segments | asr_rtf |")
    L.append("| --- | --- | --- | --- | --- |")
    L.append(
        f"| formal (Whisper) | {ba._fmt(formal_lat.p50, 2)} | {ba._fmt(formal_lat.p95, 2)} | "
        f"{formal_lat.segments} | {ba._fmt(formal_lat.asr_rtf, 2)} |"
    )
    streaming_rtf = streaming_lat.preview_rtf
    L.append(
        f"| whisper-streaming (preview) | {ba._fmt(streaming_lat.preview_p50, 2)} | "
        f"{ba._fmt(streaming_lat.preview_p95, 2)} | {streaming_lat.preview_segments} | "
    ...
    streaming_p50 = streaming_lat.preview_p50
    streaming_rtf_val = streaming_lat.preview_rtf
    L.append("### Criterion 1: Committed-caption p50 well under ~3.8 s formal median (target ~1–1.5 s)")
    if streaming_p50 is not None and formal_p50 == formal_p50:
        if streaming_p50 < 1.5:
            L.append(f"- **MET** — streaming p50 = {ba._fmt(streaming_p50, 2)} s  "
                     f"(formal p50 = {ba._fmt(formal_p50, 2)} s; "
                     f"speedup = {formal_p50 / streaming_p50:.1f}×)")
        elif streaming_p50 < formal_p50:
            L.append(f"- **PARTIAL** — streaming p50 = {ba._fmt(streaming_p50, 2)} s is faster than "
                     f"formal ({ba._fmt(formal_p50, 2)} s) but above 1.5 s target. "
                     "Follow-up: widen gate or reduce step cadence.")
        else:
            L.append(f"- **MISSED** — streaming p50 = {ba._fmt(streaming_p50, 2)} s is not faster than "
                     f"formal ({ba._fmt(formal_p50, 2)} s). "
                     "Likely two Whisper decodes competing on CPU — see RTF criterion below.")
    else:
        L.append("- **NO DATA** — streaming produced no preview segments. Check fast_preview_loop wiring.")
    L.append("")
    L.append("### Criterion 2: ASR RTF < 1 (no sustained backpressure)")
    if streaming_rtf_val is not None and streaming_rtf_val == streaming_rtf_val:
        if streaming_rtf_val < 1.0:
            L.append(f"- **MET** — streaming preview RTF = {ba._fmt(streaming_rtf_val, 2)} (< 1.0)")
        else:
            L.append(f"- **MISSED** — streaming preview RTF = {ba._fmt(streaming_rtf_val, 2)} (≥ 1.0). "
                     "Two Whisper decodes competing on the same CPU threads — "
                     "follow-up: throttle preview cadence, widen the commit gate, or pin threads.")
    else:
        L.append("- **NO DATA** — preview RTF not reported (no preview events recorded).")
    L.append("")
    L.append("## Notes")
    L.append("")
    L.append("- The formal lane is unchanged and remains the authoritative transcript source.")
    L.append("- The streaming lane uses the **same loaded model** (no second WhisperModel); decode overlap "
             "with the formal lane on a single-core CPU may inflate RTF under heavy load.")
    L.append("- Latency is measured as `wall-clock_now − audio_end_time`; preview events fire on committed words, "
             "so p50 reflects when LocalAgreement-2 first stabilises a prefix — typically after 2 growing-window steps.")
    L.append("- AMI SDM (single-distant-mic) is challenging: overlapping speakers and far-field audio inflate WER. "
             "Latency numbers are the relevant metric for this lane.")
    L.append("")
    return "\n".join(L)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--streaming-preview", action="store_true",
                   help="For the baseline knob point, also run the latency pass with StreamingWhisperAdapter "
                        "as preview_asr and report its committed-caption p50/p95 vs the formal lane. "
                        "Writes output to --out as a streaming-whisper benchmark report.")
...
async def main(argv: list[str]) -> int:
    ...
    # --streaming-preview: measure the streaming lane on the baseline knob point,
    # write a standalone report to --out, then exit (skip the regular grid sweep).
    if getattr(args, "streaming_preview", False):
        pre_vad_values_sp = _floats(args.pre_vad_max)
        accum_values_sp = _floats(args.accumulation)
        # Use the first (and typically only) knob point as the baseline.
        pvm_sp = pre_vad_values_sp[0]
        acc_sp = accum_values_sp[0]
        overrides_sp = {
            "pre_vad_max_segment_seconds": pvm_sp,
            "asr_accumulation_seconds": acc_sp,
            "asr_safeguard_enabled": False,  # isolate the knob (consistent with grid sweep default)
        }
        print(f"--streaming-preview: baseline knob point pre_vad_max={pvm_sp:g} accum={acc_sp:g}")

        # Formal-lane latency (no preview).
        print("  Running formal-lane latency pass…")
        formal_lat = await ba._run_latency(adapter, wav, extra_overrides=overrides_sp)
        ...
        print(f"\nStreaming-whisper benchmark report → {out}")
        return 0
